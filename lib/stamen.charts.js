if (typeof stamen === "undefined") stamen = {};
(function() {

    var charts = stamen.charts = {};

    charts.bars = function(parent, entries, options) {
        parent = d3.select(parent);

        if (!options) options = {};

        if (!(entries instanceof Array)) {
            entries = d3.entries(entries);
        }

        var KEY = options.key || function(entry) { return entry.key; },
            VAL = options.value || function(entry) { return entry.value; };

        var bars = parent.selectAll("div.bar")
            .data(entries, KEY);
        ...
        // operate on the final set of bars
        bars = parent.selectAll("div.bar");

        if (options.sort) {
            var cmp = (typeof options.sort === "function")
                ? options.sort
                : function(a, b) {
                    return VAL(b) - VAL(a);
                };
        ...
        var MIN = isNaN(options.min)
                ? 0
                : options.min,
            MAX = isNaN(options.max)
                ? d3.max(entries, VAL)
                : options.max,
            WIDTH = d3.scale.linear()
                .domain([MIN, MAX])
                .range([0, 100]);
        bars.style("width", function(entry) {
            return WIDTH(VAL(entry)).toFixed(1) + "%";
        });

        var COLOR = options.color || d3.scale.category10();
        bars.style("background-color", function(entry, i) {
            return COLOR(KEY(entry));
        });

        var FORMAT = options.format || d3.format(",0");
        bars.select("span.value")
            .text(function(entry) {
                return FORMAT(VAL(entry));
            });

        return bars;
    };

    charts.timeline = function(parent, data, options) {
        var $container = $(parent);

        if (!options) options = {};

        var KEY = options.key || function(d) { return d.key; },
            VAL = options.value || function(d) { return d.value; };

        var WIDTH = options.widdth || $container.width(),
            HEIGHT = options.height || 100,
            PADDING = options.padding || 10,
            XX = d3.scale.linear()
                .domain([KEY(data[0]), KEY(data[data.length - 1])])
        ...
        var line = paper.path(points.map(function(p, i) {
            ...
            .attr("fill", "none")
            .attr("stroke-width", options.stroke || 1.5)
            .attr("stroke", COLOR);

        var dots = paper.set();
        total_deaths_by_year.forEach(function(d, i) {
            var point = points[i],
    ...
    charts.util.customSort = function(sortedKeys, key) {
        var indexMap = {};
        ...
        if (typeof key !== "function") key = function(d) { return d.key; };
        return function(a, b) {
            return index(key(a)) - index(key(b));
        };
    };

    charts.util.ageSort = charts.util.customSort(["<1", "1-4", "5-14", "15-24", "25-34", "35-44", "45-54", "55-64", "65-74", "75-84", "85+"], d3.key);
